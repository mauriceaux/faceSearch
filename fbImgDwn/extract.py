...
def extractImages(page_id,  dest, access_token, website_title = "apink-official-facebook"):
    ...
    friendList = fb_public_call(page_id + "/friendlists", params, 
                                    access_token)

    while True:
        for friend in friendList["data"]:
            albums_response = fb_public_call(friend['id'] + "/albums", params,
                                     access_token);
            while True:
        ...
        if "next" in friendList["paging"]:
            next_uri = friendList["paging"]["next"]
            friendList = requests.get(next_uri).json()
        else:
            break
